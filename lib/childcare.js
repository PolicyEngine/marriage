...
// Normalize every scalar/grid scenario exactly once. Some state childcare
// programs enter household benefits already; others only enter the nationwide
// childcare aggregate. Expose one CCDF row and include the missing amount once.
export function normalizeUSChildcare(data, state, grossCost, extras = {}) {
  if (!data.child_care_subsidies) return data;
  const included = metadata.states[state]?.includedInStateBenefits;
  const payments = data.child_care_subsidies;
  const costs = data.childcare_expenses || payments.map(value => Math.max(0, grossCost - value));
  // Provider reimbursement can exceed the price paid by a family. Count only
  // the reduction in family spending as its benefit, retaining the full model
  // payment separately. The runtime preserves Vermont's collectible copay in
  // childcare_expenses, including its effects on other taxes and benefits.
  const subsidies = payments.map((value, i) => Math.min(value, Math.max(0, grossCost - costs[i])));
  const deductions = costs.map((value, i) => value + subsidies[i]);
  const adjustment = subsidies.map((value, i) => value - (included ? payments[i] : 0));
  const add = (name, values) => {
    if (data[name]) data[name] = data[name].map((value, i) => value + values[i]);
  };
  if (included) add("household_state_benefits", payments.map(v => -v));
  add("household_benefits", adjustment);
  for (const name of ["household_net_income", "household_net_income_including_health_benefits"]) {
    add(name, adjustment.map((v, i) => v - deductions[i]));
  }
  data.childcare_gross_cost = subsidies.map(() => grossCost);
  data.childcare_cost_deducted = deductions;
  data.child_care_subsidies = subsidies;
  data.childcare_provider_payment = [...payments];
  data.childcare_family_share = payments.map((value, i) => state === "VT" && value > 0 ? (data.vt_ccfap_family_share?.[i] || 0) : 0);
  data.childcare_out_of_pocket = [...costs];
  data.head_start_service_value = [...(data.head_start || subsidies.map(() => 0))];
  data.early_head_start_service_value = [...(data.early_head_start || subsidies.map(() => 0))];
  if (!extras.includeHeadStart) {
    data.head_start = subsidies.map(() => 0);
    data.early_head_start = subsidies.map(() => 0);
  }
  return data;
}
