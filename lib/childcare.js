...
export const US_CHILDCARE_SERIES = [
  "childcare_gross_cost", "childcare_out_of_pocket",
...
export function normalizeUSChildcare(data, state, grossCost, extras = {}) {
  ...
  const subsidies = data.child_care_subsidies;
  const missing = subsidies.map(value => included ? 0 : value);
  const add = (name, values) => {
    if (data[name]) data[name] = data[name].map((value, i) => value + values[i]);
  };
  if (included) add("household_state_benefits", subsidies.map(v => -v));
  add("household_benefits", missing);
  for (const name of ["household_net_income", "household_net_income_including_health_benefits"]) {
    add(name, missing.map(v => v - grossCost));
  }
  data.childcare_gross_cost = subsidies.map(() => grossCost);
  data.childcare_out_of_pocket = subsidies.map(v => Math.max(0, grossCost - v));
