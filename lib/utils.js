...
export function computeTableData(results, tab, { showHealth = false, currencySymbol = "$" } = {}) {
  ...
  if (tab === "summary") {
    ...
    // Earnings are reconstructed from net income, so anything already taken
    // out of net income has to be added back. UK net income is reported after
    // housing costs; without restoring rent here the table shows earnings
    // falling by the rent and invents a bonus for the couple, who pay one rent
    ...
    // Rent is a real cost of each household, so show it rather than leaving it
    // as an unexplained gap between the rows and the net income total.
    const showHousing = [married, headSingle, spouseSingle]
      .some((r) => (r?.aggregates?.rentDeducted || 0) > 0);
    const housing = (r) => r.aggregates.rentDeducted || 0;
    ...
    const sVals = [
      ...
      ...(showHousing ? [rsum(housing(headSingle), housing(spouseSingle))] : []),
    ];
    const hVals = [
      ...
      ...(showHousing ? [housing(headSingle)] : []),
    ];
    const spVals = [
      ...
      ...(showHousing ? [housing(spouseSingle)] : []),
    ];
    ...
    const housingRow = rows.find((r) => r.program === "Housing costs");
    if (housingRow) housingRow.rawDelta = -housingRow.rawDelta;
