...
import { addUSChildcareInputs, grossChildcareCost, normalizeUSChildcare, US_CHILDCARE_OUTPUTS, US_CHILDCARE_SERIES } from "./childcare";
...
async function callApi(countryId, situation, extras = {}) {
  const country = getCountry(countryId);
  const base = countryId === "us" ? (process.env.NEXT_PUBLIC_US_API_URL || US_API_BASE) : API_BASE;
  const res = await fetch(`${base}${country.apiPath}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ household: situation, ...(countryId === "us" ? {
      include_head_start_benefits: Boolean(extras.includeHeadStart),
...
function sumPersonVar(result, variable, year) {
  ...
  for (const person of Object.values(people)) {
    ...
    if (typeof val === "number" || typeof val === "boolean") total += Number(val);
...
function sumPersonArray(result, variable, year) {
  ...
  for (const person of Object.values(people)) {
    ...
    if (Array.isArray(val)) {
      ...
      if (!total) total = [Number(val)];
      else total[0] += Number(val);
    }
  }
  return total || [];
}
...
function extractMetaVal(countryId, result, entry, year) {
  ...
  if (!c) return 0;
  return Object.keys(result?.[c.key] || {}).reduce(
    (sum, name) => sum + extractVal(result, c.key, name, variable, year), 0,
  );
}
...
function extractMetaArray(countryId, result, entry, year) {
  ...
  const c = country.entityContainers[entity];
  if (!c) return [];
  const arrays = Object.keys(result?.[c.key] || {}).map(
    name => extractArray(result, c.key, name, variable, year),
  );
  const length = Math.max(0, ...arrays.map(arr => arr.length));
  return Array.from({ length }, (_, i) => arrays.reduce((sum, arr) => sum + (arr[i] || 0), 0));
}
...
function extractAllArrays(countryId, result, year, count = null) {
  ...
  for (const entry of allVars) {
    data[entry.variable] = normalizeIncomeGrid(extractMetaArray(countryId, result, entry, year), count);
  }
  return data;
}
...
export async function getPrograms(
  ...
  if (countryId === "us") normalizeUSChildcare(series, regionCode, grossChildcareCost(children, extras), extras);
  ...
  if (countryId === "us") {
    for (const [key, variable] of Object.entries(getCountry(countryId).aggregateMap)) {
      aggregates[key] = series[variable]?.[0] || 0;
    }
    aggregates.childcareCostDeducted = series.childcare_gross_cost?.[0] || 0;
  }
...
export async function getHeatmapData(
  ...
  if (countryId === "us") {
    const cost = grossChildcareCost(children, extras);
    normalizeUSChildcare(marriedData, regionCode, cost, extras);
    if (cohabiting) normalizeUSChildcare(unmarriedData, regionCode, cost, extras);
    else {
      normalizeUSChildcare(headData, regionCode, cost, extras);
      normalizeUSChildcare(spouseData, regionCode, 0, extras);
    }
  }
  ...
  const allEntriesForProgram = [...countryData(countryId).allVars, ...stateCreditEntries,
    ...(countryId === "us" ? US_CHILDCARE_SERIES.map(variable => ({ variable })) : []),
  ];
  const programData = {};
  for (const entry of allEntriesForProgram) {
    const v = entry.variable;
    programData[v] = {
      married: marriedData[v] || normalizeIncomeGrid(extractMetaArray(countryId, marriedResult, entry, year), count),
      ...(cohabiting ? {
        unmarried: unmarriedData[v] || normalizeIncomeGrid(extractMetaArray(countryId, unmarriedResult, entry, year), count),
      } : {
  ...
  // extras travels with the payload so buildCellResults can apply the same
  // housing-cost treatment as the grid. Without it, clicking a cell showed
  // pre-rent figures under a post-rent heatmap.
...
export function buildCellResults(
  ...
  // The grid and the metric cards report UK net income after housing costs,
  // so the selected cell has to as well. Each separate household pays the rent
  // entered, matching splitExtras in the non-heatmap path.
  const extrasFor = {
    married: extras,
    head: splitExtras(extras, "head"),
    spouse: splitExtras(extras, "spouse", { childcareCosts: 0 }),
  };

  function buildResult(scenario) {
    const aggregates = deductRent(countryId, buildAggs(scenario), extrasFor[scenario]);
    if (countryId === "us") aggregates.childcareCostDeducted = getVal("childcare_gross_cost", scenario);
    return {
      aggregates,
...
function childcareResult(value, extras) {
  return {
    ...
    includeHeadStart: Boolean(extras.includeHeadStart),
  };
}
