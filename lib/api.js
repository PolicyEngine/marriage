...
// asking: is this Universal Credit, is it child benefit, is it income tax.
// Returns the biggest movers at one cell, largest first.
export function buildCellBreakdown(
  countryId, programData, headIdx, spouseIdx, count, limit = 4,
) {
  const { m } = countryData(countryId);
  ...
  // Taxes are a cost, so a rise in tax is a fall in income. Flipping the sign
  // here lets every row read the same way: positive favours living together.
  const groups = [
    { entries: m.benefits || [], sign: 1 },
    { entries: m.credits || [], sign: 1 },
    { entries: m.taxes || [], sign: -1 },
  ];

  const rows = [];
  for (const { entries, sign } of groups) {
    for (const entry of entries) {
      const together = valueAt(entry.variable, "married");
      const apart = valueAt(entry.variable, "head") + valueAt(entry.variable, "spouse");
