...
export async function getHeatmapData(
  ...
  const [marriedResult, headResult, spouseResult, unmarriedResult] = await Promise.all([
    ...
    cohabiting ? null : callApi(countryId, headSingleSituation),
    cohabiting ? null : callApi(countryId, spouseSingleSituation),
    cohabiting ? callApi(countryId, unmarriedSituation) : null,
  ]);
...
export function buildCellResults(
  ...
  function buildResult(scenario) {
    return {
      aggregates: deductRent(countryId, buildAggs(scenario), extrasFor[scenario]),
